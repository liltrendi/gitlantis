...
import { PLAYER_MODEL_PATH } from "@/browser/config";
...
export const addPlayerToWorld = ({
  ...
  gltfModelLoader.load(PLAYER_MODEL_PATH, (loadedModel: GLTF) => {
    ...
    const initialAnimation = playerAnimationMixer.clipAction(
      playerModel.animations.find((a) => a.name === PLAYER_ANIMATIONS.initial)!
    );
